...
CREATE TABLE animals (
   id SERIAL PRIMARY KEY,
   species_id INTEGER FOREIGN KEY REFERENCES species (id),
   nickname VARCHAR
);
...
CREATE TABLE sightings (
   id SERIAL PRIMARY KEY,
   researcher_id INTEGER FOREIGN KEY REFERENCES researchers (id) ON DELETE SET NULL,
   species_id INTEGER FOREIGN KEY REFERENCES species (id) ON DELETE CASCADE,
   habitat_id INTEGER FOREIGN KEY REFERENCES habitat (id)
);
